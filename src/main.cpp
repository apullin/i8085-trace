...
static bool ParseIRQ(const char* str, ScheduledIRQ* irq)
{
    ...
    {
        for (auto& c : codeStr) c = (char)tolower(c);
...
static void OutputTrace(FILE* out, const TraceState& t, const State8085* state)
{
    ...
    fprintf(out, "\"%02X\",\"%02X\",\"%02X\",\"%02X\",\"%02X\",\"%02X\",\"%02X\"",
...
int main(int argc, char* argv[])
{
    ...
    if (cfg.summary)
    {
        fprintf(out, "{\"pc\":\"%04X\",\"sp\":\"%04X\",\"f\":\"%02X\",\"clk\":%" PRIu64 ",\"steps\":%" PRIu64 ",\"halt\":\"%s\",\"r\":[",
